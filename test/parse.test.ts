...
test('parseImageAssetId(): throws on invalid document id (image getter)', () => {
  expect(() => parseImageAssetId('moop')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed asset ID 'moop'. Expected an id like "image-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-2000x3000-jpg".]`,
  )
  expect(() => parseImageAssetId('image-hash-300x-200-png')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed asset ID 'image-hash-300x-200-png'. Expected an id like "image-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-2000x3000-jpg".]`,
  )
  expect(() => parseImageAssetId('image-hash-fooxbar-png')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed asset ID 'image-hash-fooxbar-png'. Expected an id like "image-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-2000x3000-jpg".]`,
  )
  expect(() => parseImageAssetId('image-hash-20x-png')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed asset ID 'image-hash-20x-png'. Expected an id like "image-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-2000x3000-jpg".]`,
  )
})
...
test('parseFileAssetId(): throws on invalid document id (file getter)', () => {
  expect(() => parseFileAssetId('moop')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed file asset ID 'moop'. Expected an id like "file-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-pdf"]`,
  )
  expect(() => parseFileAssetId('file-hash-300x-200-png')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Malformed file asset ID 'file-hash-300x-200-png'. Expected an id like "file-027401f31c3ac1e6d78c5d539ccd1beff72b9b11-pdf"]`,
  )
})
...
test('parseAssetFilename(): throws on invalid asset filenames', () => {
  expect(() => parseAssetFilename('blatti')).toThrowErrorMatchingInlineSnapshot(
    `[Error: Invalid image/file asset filename: blatti]`,
  )
})
...
test('parseAssetUrl(): throws on invalid URLs', () => {
  expect(() => parseAssetUrl('https://not.sanity.url')).toThrowErrorMatchingInlineSnapshot(
    `[Error: URL is not a valid Sanity asset URL: https://not.sanity.url]`,
  )

  expect(() =>
    parseAssetUrl('https://cdn.sanity.io/studios/foo.jpg'),
  ).toThrowErrorMatchingInlineSnapshot(
    `[Error: URL is not a valid Sanity asset URL: https://cdn.sanity.io/studios/foo.jpg]`,
  )
  ...
  expect(() =>
    ...
    `[Error: URL is not a valid Sanity asset URL: https://cdn.sanity.io/files/espenhov/e5p3n+lol/ae0ef9f916843d32fef3faffb9a675d4cce046f0.pdf]`,
  )
  ...
  expect(() =>
    ...
    `[Error: URL is not a valid Sanity asset URL: https://cdn.sanity.io/files/espen#hov/diary/ae0ef9f916843d32fef3faffb9a675d4cce046f0.pdf]`,
  )
